...
for iDepl = 1:length(deplList)
    ...
    for fidx = 1:size(FileList,1) % Make sure to change the start of the file list so that it begins with the first file
        ...
        for idx = 1:inc
            ...
            start = ((PARAMS.xhd.byte_loc(idx)-PARAMS.xhd.byte_loc(1))/2)+1;%round(rawDur(idx)*fs);
            if idx ==inc
                stop = PARAMS.xhd.dSubchunkSize/2;
            else
                stop = round((PARAMS.xhd.byte_loc(idx+1)-PARAMS.xhd.byte_loc(1)-1)/2);
            end            
            step = (stop - start);
                ...
                if ~isempty(expConv)
                    ...
                    for eidx = 1:size(expConv,1)
                        ...
                        if ~isempty(expTimes)
                            ...
                            corrValSet = [corrValSet;myCorrVal];
                        end
                    ...
                    allCorrVal = [allCorrVal; corrValSet];
